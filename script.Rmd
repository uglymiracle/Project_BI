...
#CX data
CX <- read.csv('data/INT.CX.YeastNet.v3.5730gene.242504link.txt', sep='\t', header = F) %>% 
  as.data.table()
```
...
names(ppi_ji_01)[c(1,2)] <- c('Systematic.Name.Interactor.A', 
                              'Systematic.Name.Interactor.B')
```
...
```{r}
names(CX) <- c('Systematic.Name.Interactor.A', 'Systematic.Name.Interactor.B', 'Score')
...
CX_PPI <- inner_join(CX, PPI_uniq[,c(6,7,12, 19)], 
                               PPI_uniq[,c(6,7,12,19)] %>% 
                                 dplyr::mutate(A = Systematic.Name.Interactor.B,
                                               B = Systematic.Name.Interactor.A,
                                               Systematic.Name.Interactor.A = A,
                                               Systematic.Name.Interactor.B = B),   
...
### JI
```{r}
union_CX_3netw <- dplyr::union(costanzo_02[,c(1,2)], PPI[, c(6, 7)]) %>% 
  dplyr::union(CX[,c(1,2)])

mask.dup <-
  paste(union_CX_3netw$Systematic.Name.Interactor.A, 
        union_CX_3netw$Systematic.Name.Interactor.B, sep = ',') %>%
  str_split(',') %>% 
  lapply(function(x) x %>% 
           sort %>% 
           paste(collapse = ',')) %>%
...
union_CX_3netw <- union_CX_3netw[!mask.dup, ]
...
CX_JI <- CX_GI_PPI[,1]  %>% 
  ...
  unique() %>% 
  mutate(OI = sapply(Systematic.Name.Interactor.A, function(x)
                             ((sum(CX_GI_PPI$Systematic.Name.Interactor.A == x) +
                             sum(CX_GI_PPI$Systematic.Name.Interactor.B == x)) / 
                               (sum(PPI_uniq$Systematic.Name.Interactor.A == x) +
                                  sum(PPI_uniq$Systematic.Name.Interactor.B  == x))))) %>% 
  mutate(JI = sapply(Systematic.Name.Interactor.A, function(x)
                             ((sum(CX_GI_PPI$Systematic.Name.Interactor.A == x) +
                             sum(CX_GI_PPI$Systematic.Name.Interactor.B == x)) / 
                               (sum(union_CX_3netw$Systematic.Name.Interactor.A == x) +
                                  sum(union_CX_3netw$Systematic.Name.Interactor.B == x))))) 
...
ego_3netw <- enrichGO(annotate_3netw$ENTREZID, 
                      universe = annotate_data_all$ENTREZID, 
